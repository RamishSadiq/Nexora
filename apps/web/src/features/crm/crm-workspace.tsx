...
export function CrmWorkspace({ permissions, initial, initialRecord }: { permissions: string[]; initial: ListState; initialRecord: string }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-5 p-4 text-slate-800 sm:p-7">
    ...
    <section aria-label="CRM records" className="overflow-hidden rounded-xl border border-slate-200 bg-white" aria-busy={loading}>
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">{query.kind === "contact" ? "Contacts" : "Accounts"} matching current filters</caption><thead className="bg-slate-50 text-slate-600"><tr><th scope="col" className="p-4">Name</th>{columns.includes("status") && <th scope="col" className="p-4">Status</th>}{columns.includes("category") && <th scope="col" className="p-4">Category</th>}{columns.includes("updatedAtUtc") && <th scope="col" className="p-4">Updated</th>}<th scope="col" className="p-4 text-right">Actions</th></tr></thead><tbody>{!loading && items.map(row => <tr key={row.id} className="border-t border-slate-100"><th scope="row" className="p-4"><button className="text-left font-semibold text-indigo-700 underline-offset-4 hover:underline" onClick={() => { setSelected(row.id); setCreating(false); }}>{row.name}</button></th>{columns.includes("status") && <td className="p-4">{row.status}</td>}{columns.includes("category") && <td className="p-4">{row.category ?? "—"}</td>}{columns.includes("updatedAtUtc") && <td className="p-4">{new Date(row.updatedAtUtc).toLocaleDateString()}</td>}<td className="p-4"><div className="flex justify-end gap-1"><button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`View ${row.name}`} title="View" onClick={() => { setSelected(row.id); setCreating(false); }}><Eye className="size-4" /></button>{canManage && <button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`Edit ${row.name}`} title="Edit" onClick={() => { setSelected(row.id); setCreating(false); }}><Pencil className="size-4" /></button>}{canManage && <span className="rounded-lg p-2 text-slate-300" title="Archive from record details"><Archive className="size-4" /></span>}</div></td></tr>)}</tbody></table></div>
    ...
    {permissions.includes("crm.configure") && <Card title="Define a custom field"><p className="mb-3 text-sm text-slate-600">Fields have a fixed data type. Create up to 50 definitions for your tenant.</p><form className="grid items-end gap-3 sm:grid-cols-4" onSubmit={e => { const r = data(e); const form = e.currentTarget; void perform(async () => { await crm("/fields", "POST", r); form.reset(); }); }}><Label title="Field name"><input name="name" className={input} maxLength={80} required /></Label><Label title="Applies to"><select name="kind" className={input}><option value="contact">Contacts</option><option value="account">Accounts</option></select></Label><Label title="Type"><select name="dataType" className={input}>{["text", "number", "boolean", "date"].map(t => <option key={t}>{t}</option>)}</select></Label><button className={primary} disabled={busy}>Create field</button></form></Card>}
  </main>;
}
