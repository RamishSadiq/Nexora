"use client";
import { AttributeFields, attributeValues } from "../shared/attribute-fields";

import { useEffect, useId, useRef, useState, type FormEvent, type ReactNode } from "react";
import { Eye, ListFilter, MoreHorizontal, Pencil, Trash2, UserRound } from "lucide-react";
import { crm } from "./api";
...
export function CrmWorkspace({ permissions, initial, initialRecord, fixedKind = false }: { permissions: string[]; initial: ListState; initialRecord: string; fixedKind?: boolean }) {
  ...
  const [columns, setColumns] = useState<string[]>(["name", "status", "category"]);
  ...
  useEffect(() => {
    let current = true;
    Promise.all([crm<Directory>("/directory"), crm<Field[]>("/fields")]).then(([d, f]) => { if (current) { setDirectory(d); setFields(f); } }).catch(e => { if (current) setError(e.message); });
    return () => { current = false; };
  }, [refresh]);
  ...
  if (creating) return <main className="page-enter mx-auto max-w-4xl space-y-6 p-5 sm:p-8"><header className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-indigo-600">CRM workspace</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Create {query.kind}</h1><p className="mt-2 text-sm text-slate-600">Complete the available details for this {query.kind} before saving.</p></div><button className={button} onClick={() => setCreating(false)}>Back to {query.kind === "contact" ? "contacts" : "accounts"}</button></header><Card title={`${query.kind === "contact" ? "Contact" : "Account"} details`}><RecordForm kind={query.kind} directory={directory} busy={busy} onCancel={() => setCreating(false)} onSave={r => perform(async () => { const saved = await crm<RecordRow>("/records", "POST", r); setCreating(false); setSelected(saved.id); })} /></Card></main>;
  if (selected) return <main className="page-enter mx-auto max-w-7xl space-y-5 p-5 sm:p-8"><button className={button} onClick={() => setSelected("")}>← Back to {query.kind === "contact" ? "contacts" : "accounts"}</button><RecordDetail key={`${selected}-${selectedReadOnly}`} id={selected} directory={directory} fields={fields} permissions={permissions} readOnly={selectedReadOnly} onChanged={() => setRefresh(r => r + 1)} onClose={() => setSelected("")} /></main>;
  return <main className="mx-auto max-w-7xl space-y-5 p-4 text-slate-800 sm:p-7">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-indigo-600">CRM workspace</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{query.kind === "contact" ? "Contacts" : "Accounts"}</h1><p className="mt-1 text-sm text-slate-600">People, organisations and the history that connects them.</p></div><div className="flex items-center gap-2">{canManage && <button className={primary} onClick={() => { setCreating(true); setSelected(""); }}>Create {query.kind}</button>}<button className={button} onClick={() => setFilterViewOpen(true)}><ListFilter className="size-4" />Filter view</button></div></header>
    ...
    <section aria-label="Search records" className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
      ...
      <form className="flex max-w-2xl items-end gap-2" onSubmit={e => { e.preventDefault(); filter({ search }); }}><Label title="Search by name"><input className={input} value={search} maxLength={160} onChange={e => setSearch(e.target.value)} placeholder="Find a person or organisation" /></Label><button className={button}>Search</button></form>
    </section>
    {filterViewOpen && <aside className="fixed inset-y-0 right-0 z-40 w-full max-w-sm overflow-y-auto border-l border-slate-200 bg-white p-6 shadow-2xl" aria-label="Filter view panel"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-indigo-600">Listing layout</p><h2 className="mt-1 text-xl font-semibold">Filter view</h2></div><button className={button} onClick={() => setFilterViewOpen(false)} aria-label="Close filter view">Close</button></div><p className="mt-3 text-sm text-slate-600">Choose which attributes appear as columns on this listing.</p><div className="mt-6 space-y-3">{[{ id: "name", label: "Name" }, { id: "status", label: "Status" }, { id: "category", label: "Category" }, ...fields.filter(f => f.kind === query.kind).map(f => ({ id: `field:${f.id}`, label: `${f.name} (${f.dataType})` }))].map(column => <label key={column.id} className="flex items-center gap-3 rounded-lg border border-slate-200 p-3 text-sm"><input type="checkbox" checked={columns.includes(column.id)} onChange={() => setColumns(current => current.includes(column.id) ? current.filter(x => x !== column.id) : [...current, column.id])} />{column.label}</label>)}</div></aside>}
    <section aria-label="CRM records" className="overflow-hidden rounded-xl border border-slate-200 bg-white" aria-busy={loading}>
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">{query.kind === "contact" ? "Contacts" : "Accounts"} matching current filters</caption><thead className="bg-slate-50 text-slate-600"><tr><th scope="col" className="w-12 p-4"><span className="sr-only">Select</span><input type="checkbox" aria-label="Select all records" /></th><th scope="col" className="p-4">Actions</th>{columns.includes("name") && <th scope="col" className="p-4">Name</th>}{columns.includes("status") && <th scope="col" className="p-4">Status</th>}{columns.includes("category") && <th scope="col" className="p-4">Category</th>}{fields.filter(f => f.kind === query.kind && columns.includes(`field:${f.id}`)).map(f => <th key={f.id} scope="col" className="p-4">{f.name}</th>)}</tr></thead><tbody>{!loading && items.map(row => <tr key={row.id} className="border-t border-slate-100 hover:bg-indigo-50/30"><td className="p-4"><input type="checkbox" aria-label={`Select ${row.name}`} /></td><td className="p-4"><div className="flex justify-start gap-1"><button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`View ${row.name}`} title="View" onClick={() => { setSelected(row.id); setSelectedReadOnly(true); setCreating(false); }}><Eye className="size-4" /></button>{canManage && <button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`Edit ${row.name}`} title="Edit" onClick={() => { setSelected(row.id); setSelectedReadOnly(false); setCreating(false); }}><Pencil className="size-4" /></button>}{canManage && <button className="rounded-lg p-2 text-rose-500 hover:bg-rose-50 hover:text-rose-700" aria-label={`Delete ${row.name}`} title="Delete" onClick={() => void perform(async () => { await crm(`/records/${row.id}/archive`, "POST", { version: row.version }); })}><Trash2 className="size-4" /></button>}</div></td>{columns.includes("name") && <th scope="row" className="p-4"><button className="flex items-center gap-3 text-left font-semibold text-slate-800 hover:text-indigo-700" onClick={() => { setSelected(row.id); setSelectedReadOnly(true); setCreating(false); }}><span className="grid size-8 place-items-center rounded-full bg-slate-100 text-slate-500"><UserRound className="size-4" /></span>{row.name}</button></th>}{columns.includes("status") && <td className="p-4">{row.status}</td>}{columns.includes("category") && <td className="p-4">{row.category ?? "—"}</td>}{fields.filter(f => f.kind === query.kind && columns.includes(`field:${f.id}`)).map(f => { const v = row.customFields?.find(x => x.definitionId === f.id); return <td key={f.id} className="p-4">{v?.textValue ?? v?.numberValue ?? (v?.booleanValue === null || v?.booleanValue === undefined ? "—" : v.booleanValue ? "Yes" : "No") ?? "—"}</td>; })}</tr>)}</tbody></table></div>
    ...
    {selected && <RecordDetail key={selected} id={selected} directory={directory} fields={fields} permissions={permissions} readOnly={selectedReadOnly} onChanged={() => setRefresh(r => r + 1)} onClose={() => setSelected("")} />}
  </main>;
}
...
function RecordForm({ row, kind, directory, busy, onSave, onCancel }: { row?: RecordRow; kind: string; directory: Directory; busy: boolean; onSave: (r: unknown) => Promise<void>; onCancel: () => void }) {
  return <form className="grid gap-4 sm:grid-cols-2" onSubmit={e => { const r = data(e); const attributes = attributeValues(r); void onSave({ ...r, attributes, kind, category: r.category || null, ownerUserId: r.ownerUserId || null, ownerTeamId: r.ownerTeamId || null, version: row?.version }); }}>
    ...
    <AttributeFields kind={kind} recordId={row?.id} />
...
function RecordDetail({ id, directory, fields, permissions, readOnly, onChanged, onClose }: { id: string; directory: Directory; fields: Field[]; permissions: string[]; readOnly: boolean; onChanged: () => void; onClose: () => void }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`nexora-layout-${detail?.record.kind === "contact" ? "contacts" : "accounts"}`);
    if (!saved || !layoutRef.current) return;
    const order = JSON.parse(saved) as string[];
    Array.from(layoutRef.current.children).forEach((child, index) => {
      const title = child.querySelector("h2")?.textContent ?? "";
      (child as HTMLElement).style.order = String(order.indexOf(title) >= 0 ? order.indexOf(title) : order.length + index);
    });
  }, [detail]);
  ...
  const editable = manage && !readOnly && detail && !detail.record.isArchived;
