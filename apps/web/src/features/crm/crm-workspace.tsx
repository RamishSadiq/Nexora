...
export function CrmWorkspace({ permissions, initial, initialRecord, fixedKind = false }: { permissions: string[]; initial: ListState; initialRecord: string; fixedKind?: boolean }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-5 p-4 text-slate-800 sm:p-7">
    ...
    <section aria-label="CRM records" className="overflow-hidden rounded-xl border border-slate-200 bg-white" aria-busy={loading}>
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">{query.kind === "contact" ? "Contacts" : "Accounts"} matching current filters</caption><thead className="bg-slate-50 text-slate-600"><tr><th scope="col" className="w-12 p-4"><span className="sr-only">Select</span><input type="checkbox" aria-label="Select all records" /></th><th scope="col" className="p-4">Name</th>{columns.includes("status") && <th scope="col" className="p-4">Status</th>}{columns.includes("category") && <th scope="col" className="p-4">Category</th>}<th scope="col" className="p-4 text-right">Actions</th></tr></thead><tbody>{!loading && items.map(row => <tr key={row.id} className="border-t border-slate-100 hover:bg-indigo-50/30"><td className="p-4"><input type="checkbox" aria-label={`Select ${row.name}`} /></td><th scope="row" className="p-4"><button className="flex items-center gap-3 text-left font-semibold text-slate-800 hover:text-indigo-700" onClick={() => { setSelected(row.id); setSelectedReadOnly(true); setCreating(false); }}><span className="grid size-8 place-items-center rounded-full bg-slate-100 text-slate-500"><UserRound className="size-4" /></span>{row.name}</button></th>{columns.includes("status") && <td className="p-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${row.status === "active" ? "bg-emerald-100 text-emerald-800" : row.status === "prospect" ? "bg-amber-100 text-amber-800" : "bg-rose-100 text-rose-800"}`}>{row.status}</span></td>}{columns.includes("category") && <td className="p-4"><span className="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{row.category ?? "—"}</span></td>}<td className="p-4"><div className="flex justify-end gap-1"><button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`View ${row.name}`} title="View" onClick={() => { setSelected(row.id); setSelectedReadOnly(true); setCreating(false); }}><Eye className="size-4" /></button>{canManage && <button className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700" aria-label={`Edit ${row.name}`} title="Edit" onClick={() => { setSelected(row.id); setSelectedReadOnly(false); setCreating(false); }}><Pencil className="size-4" /></button>}{canManage && <button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label={`More actions for ${row.name}`} title="More actions" onClick={() => { setSelected(row.id); setSelectedReadOnly(true); setCreating(false); }}><MoreHorizontal className="size-4" /></button>}</div></td></tr>)}</tbody></table></div>
